#!/usr/bin/env bash
# Battlefield-6-launcher.sh
#
# Boots into Windows via systemd-boot and auto-launches Battlefield 6 on Windows startup.
#
# Flow:
...
# Example: "Program Files/EA Games/Battlefield 6/BF6.exe"
...
WINDOWS_MOUNT="/mnt/windows_bf6_launch"
...
LAUNCHER_BAT="battlefield6_launch.bat"
DRY_RUN=0

# ---- helpers -----------------------------------------------

die()  { echo "ERROR: $*" >&2; exit 1; }
info() { echo "[bf6-launch] $*"; }
warn() { echo "WARNING: $*" >&2; }
...
write_startup_bat() {
    ...
    if [[ -n "$GAME_EXE_WINDOWS_PATH" ]]; then
        ...
        cat > "$bat_path" << BATEOF
@echo off
REM Battlefield-6-launcher: one-shot game launcher (direct exe)
REM This file was created by Battlefield-6-launcher.sh on Linux.
...
BATEOF
        ...
        cat > "$bat_path" << BATEOF
@echo off
REM Battlefield-6-launcher: one-shot Steam game launcher
REM This file was created by Battlefield-6-launcher.sh on Linux.
REM It will delete itself after launching the game.
